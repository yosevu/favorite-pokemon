...
<img 
  ...
  src=${`https://assets.pokemon.com/assets/cms2/img/pokedex/full/00${monster.index}.png`}
  alt="Bulbasaur"/>
`;

const MonsterHeart = (monster) => (monster.liked) ?  likedHeart : unlikedHeart;

const MonsterIndex = (monster) => html`
<div class="monster__index">#00${monster.index}</div>
`;

const Monster = (monster) => html`
<li class="monster">
  ${MonsterName(monster)}
...
<ul class="monsters">
 ${ monsters.map((monster, index) => Monster({...monster, index: index+1}))}
</ul>
`;
